...
export default function HabitsPage() {
  ...
  return <div className="mx-auto max-w-2xl space-y-6"><h1 className="text-3xl font-semibold">Today’s Habits</h1><section className="card grid gap-4"><Field label="Water intake ml" type="number" value={habit.waterIntakeMl} onChange={(v) => setHabit({ ...habit, waterIntakeMl: Number(v) })}/><Field label="Sleep hours" type="number" value={habit.sleepHours} onChange={(v) => setHabit({ ...habit, sleepHours: Number(v) })}/><label className="flex items-center gap-3"><input type="checkbox" checked={habit.usedSpf} onChange={(e) => setHabit({ ...habit, usedSpf: e.target.checked })}/> Used SPF today</label><Field label="Stress 1-5" type="number" value={habit.stressLevel} onChange={(v) => setHabit({ ...habit, stressLevel: Number(v) as DailyHabit["stressLevel"] })}/><Field label="Exercise minutes" type="number" value={habit.exerciseMinutes ?? 0} onChange={(v) => setHabit({ ...habit, exerciseMinutes: Number(v) })}/><textarea className="input" placeholder="Notes" value={habit.notes} onChange={(e) => setHabit({ ...habit, notes: e.target.value })}/><button className="btn-primary" onClick={save}>Save Today’s Log</button></section></div>;
}
